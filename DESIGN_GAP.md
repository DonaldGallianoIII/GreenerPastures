...
| **Harvester** (passive drops → its own chest, never on the ground) | 🟢 built | The **Harvester block** (`drops/`): rolls each tethered mon's Cobblemon drop table (`DropsBridge`) via our tested `DropTable`/`DropEntry`, deposits into a vanilla 9×3 container, pause-when-full, scatter-on-break. Replaces the cut loot-sweep. **Remaining = Drop Rate/Yield augment modulation (cadence/yield) + species-combo easter eggs.** Materials only, never Data. |
