...
| **Augment-on-Kernel** (base mods as data) | ✅ catalog | `augments` component now holds the full `{function→level}` catalog (back-compat). **All seven v1 functions** have live effects (base + tether-amplified, each drained by its own consumer): Shiny / Speed / IV Floor / EV at the breeder, Drop Rate / Drop Yield at the Harvester, Enrichment at the Renderer. |
| **Compiler** (writes mods) | ✅ / 🔴 | Installs the **Shiny / Speed / Enrichment / Drop Rate / Drop Yield** augments onto a Kernel (generic over `AugmentFunction` — live preview + Compile button, inputs returned on close/disconnect). Design also wants it to **inscribe Soul Tethers** — not built. |
...
| **More augments** (IV Floor, Fine-Tune/EV, Enrichment, Drop Rate, Drop Yield = v1; Nature/Egg-Move/Ability/Gender/Hatch-Speed/Clutch/Anomaly = backlog) | 🟢 | **All seven v1 functions** (Shiny, Speed, IV Floor, EV, Enrichment, Drop Rate, Drop Yield) are craftable Compiler augments with live effects + tether-amp. IV Floor guarantees N perfect IVs; EV pre-sets a flat per-stat EV floor (both carried to the hatchling via Cobbreeding's `PokemonProperties.apply`). Nature/Egg-Move/Ability/Gender/Hatch-Speed/Clutch/Anomaly = backlog. |
