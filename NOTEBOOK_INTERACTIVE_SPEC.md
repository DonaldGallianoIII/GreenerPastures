...
### 3.3 BioBank  ✅ *6a BUILT (per-player store + browse accordion); 6b = full-card EggReader extension*
...
6. **BioBank tab** — **split into 6a + 6b.**
   ...
   - [ ] **6b (next):** extend `EggReader` → the full competitive card (per-stat IVs · EVs · nature · ability+hidden · Tera · ball · OT · moves) + search/sort. The reflection work (mirror `CobbreedingBridge`'s `PokemonProperties` getters).
