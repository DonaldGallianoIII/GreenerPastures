...
### 3.2 Pastures  🟡
- **Reads:** enumerate owned = walk `server.getWorlds()` × `PastureRegistry.get(server).inWorld(w)` filtering `pd.owner == uuid` (no per-UUID index exists) → per-pasture `{dim, pos, name, tier(), pairs/maxPairs, eggQueue.size()}`. For a **selected, chunk-loaded** pasture: `CobbreedingBridge.rosterOf(be, pd)` → `List<MonEntry(id,species,label,bucket)>` (the 8-pair arrangement); per-pair status derived (bucket has 2 mons → **Breeding** if `now < pd.nextBreedTick` & `isBreedingActivated`, **Ready** if `now ≥ nextBreedTick`, else **Idle**); progress ≈ `1 − (nextBreedTick − now)/interval`; effective augments via `TetherRuntime.resolveFor(pd.baseAugmentLevels(), pd.slottedTethers(), DataStore.balanceOf(owner), {breeding fns})`; tether cost/cycle via `TetherRuntime.totalBurn(pd.slottedTethers())`.
- **Actions:** `CLAIM_PASTURE(dim,pos)` toggle link/unlink (⚠️ existing `ClaimOperatorPayload`→`PastureNet.onClaim` is **reach-gated ~64 blk** — see §7 Q1).
- **Hooks:** `PastureRegistry`, `PastureData`, `CobbreedingBridge.rosterOf/eggsAt/isBreedingActivated/nextBreedingInterval`, `MultiPairBreeder.bucketPairs/speedAdjustedInterval` (mirror the pairing+interval logic), `PastureClaim.toggle`, `TetherRuntime`, `DataStore`. Read-and-ship template: `PastureWand.openMenu`→`PastureOpenData`; `MonEntry` already has a `PacketCodec`.
- **owo:** left = scroll list of owned pastures (name · ready-dot · `n/8` · dim tag); right = selected → 8-pair grid (2-col) each `parentA × parentB` + status color + progress bar, empty = `+ add pair`; header name + tier + effective drop/shiny + tether cost/cycle; a **Link/Unlink** button.
- **Build state:** registry + roster + claim all exist; needs the per-UUID walk, a new S2C pasture-list + detail payload, and the remote-link/remote-roster decision (Q1).
...
### 3.5 Augmenter (Kernel)  🔴 *a backend build, not a skin*
...
4. **Pastures tab** — enumeration walk + roster/status payload + link (needs Q1).
5. **Augmenter backend + tab** — build the slot/GPU/Data/remove model, then triptych (needs Q4, Q5).
