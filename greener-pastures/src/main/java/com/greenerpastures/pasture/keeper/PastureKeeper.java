 ...
 *
 * <p>Per-pasture no-wander: the pasture ticker calls {@code togglePastureOn(viewersInRange > 0)} to
    ...
    public static void init() {
        UseBlockCallback.EVENT.register((player, world, hand, hit) -> {
            ...
            if (!world.isClient) {
                ...
                boolean now = toggle(p);
                Analytics.record(world, Event.of("pasture_toggle")
                        .put("x", p.getX()).put("y", p.getY()).put("z", p.getZ())
                        .put("suppressed", now)
                        .player(player.getUuid()));
                player.sendMessage(Text.literal("[Greener Pastures] this pasture — wandering "
                        + (now ? "OFF (mons stay in the box)" : "ON")), false);
            }
