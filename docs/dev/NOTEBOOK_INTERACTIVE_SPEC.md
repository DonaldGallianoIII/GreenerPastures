...
### 3.3 BioBank  ✅ *6a + 6b BUILT (per-player store + accordion + full competitive card via EggReader.card)*
...
6. **BioBank tab** — **split into 6a + 6b.**
   - ✅ **6a BUILT + green (2026-06-30):** per-player migration (`BioBankStore` block-keyed → **UUID-keyed**; the BioBank block is now a deposit station that fills *your* bank, no scatter-on-break) + `BioBankData.entries()` + `NotebookBioBankS2C` + a **species accordion** (species → count → expand → per-egg shiny★ / IV total / # perfect). Browse-only. *(Migration note: legacy block-keyed banks drop on load — re-deposit.)*
   - ✅ **6b BUILT + green (2026-06-30):** `EggReader.card()` reads an egg's full spec via graceful reflection — per-stat IVs[6] + EVs[6] (`statSlot` name-matching) + nature + gender + ability; new `EggCard` + rich `NotebookBioBankS2C.Entry` (manual `PacketCodec.ofStatic`, since 10 fields > tuple's 6). Tab entry = a rich card (meta line + IV chips + EV chips when present). Tera/ball/OT/moves omitted (post-hatch — BioBank = eggs). **QA-in-morning:** verify which reflection getters populate — `nature`/`gender`/`ability`/`getEvs` are best-guess method names; any blank field = the real `PokemonProperties` getter differs (fix the name). Also verify per-stat IV/EV **order** (statSlot vs Cobblemon's `Stat` names). Deferred: search/sort.
